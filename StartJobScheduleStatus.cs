...
namespace InstallerApp
{
    ...
    public class StartJobScheduleStatus : Activity
    {
        ...
        PhonegapWebService.phonegap serviceInstaller;
        string[] getStrings;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            //Call WebService
            serviceInstaller = new PhonegapWebService.phonegap();
            ...
            if (int.Parse(getStrings[7]) == 0) { //If InstallerJobStatus = 0, then first start job and do something
                headerGeneralInfo.imgViewIcon.SetImageResource(Resource.Drawable.imgSchedule_Trans);
                imgbtnStartJob.Click += delegate
                {
                    funStartingJob(0);
                    headerGeneralInfo.imgViewIcon.SetImageResource(Resource.Drawable.imgProgress_Trans);
                };
            }
        ...
        public void displayTableLayout()
        {
            ...
            for (int i = 0; i < serviceListRoomsInfo.Length; i++)
            {
                ...
                {
                    int k = btn.Id;
                    string[] strRooms = new string[] { serviceListRoomsInfo[k].Rooms, serviceListRoomsInfo[k].Style, serviceListRoomsInfo[k].Colour, serviceListRoomsInfo[k].Hardware, serviceListRoomsInfo[k].CounterTop };
                    Bundle b = new Bundle();
